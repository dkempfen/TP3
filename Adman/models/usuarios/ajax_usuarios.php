...
if(!empty($_POST)) /*si esta vacia*/{
    if(empty($_POST['nombre']) || empty($_POST['usuario']) || empty($_POST['clave'])){
        ...
        $sql = "SELECT * FROM usuarios where usuario = '$usuario' and usuario_id ='$idusuario'";
        ...
        if($result > 0) {
            $respuesta = array('status' => false, 'msg'=> 'El usuarios ya existe');
             ...
             if ($idusuario == 0){
                ...
                if(empty($clave)){
                    $sqlUpdate = "UPDATE usuarios SET nombre='$nombre', usuario='$usuario', rol='$rol', estado='$estado' WHERE usuario_id = '$idusuario'";
